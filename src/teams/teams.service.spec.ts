...
describe('TeamsService', () => {
  ...
  describe('create', () => {
    ...
    it('should throw if team number is not unique', async () => {
      ...
      await expect(
        service.createTeam({ name: 'Team 1', teamNumber: '000001' } as any),
    ...
    it('should throw BadRequestException on prisma error', async () => {
      ...
      await expect(
        service.createTeam({ name: 'Team 1' } as any),
  ...
  describe('update', () => {
    ...
    it('should throw NotFoundException if team does not exist', async () => {
      prisma.team.findUnique.mockResolvedValue(null);
      await expect(
        service.update('notfound', { name: 'fail' } as any),
    ...
    it('should throw BadRequestException on prisma error', async () => {
      ...
      await expect(
        service.update('team1', { name: 'fail' } as any),
      ).rejects.toThrow('Failed to delete team: DB error');
    });
  });
  ...
  describe('importTeams', () => {
    ...
    it('should throw BadRequestException if no data', async () => {
      ...
      await expect(service.importTeams(dto as any)).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('should skip teams with errors and continue', async () => {
      ...
      prisma.team.findMany.mockResolvedValue([]);
      prisma.team.findUnique.mockResolvedValue(null);
      prisma.team.create.mockImplementationOnce(() => {
        throw new Error('fail');
      });
      prisma.team.create.mockResolvedValue(
        createMockTeam({ id: 'team2', name: 'Team 2' }) as any,
      );
      const dto = {
        content: 'Team 1,Org,Desc\nTeam 2,Org2,Desc2',
        format: 'csv',
